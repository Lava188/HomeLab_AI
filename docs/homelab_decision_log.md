...
| Do not switch default runtime while semantic is inactive | Semantic activation audit shows `semanticScore = 0` and `runtimeMode = lexical_only`. | Default switch would falsely claim semantic/hybrid retrieval. | v1_3 may be used only in controlled testing until semantic retrieval is real and verified. | BLOCKED default switch |
